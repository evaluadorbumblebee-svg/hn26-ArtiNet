import { NextRequest } from 'next/server'
import { createClient } from '@/lib/supabase/server'
import { getAuthUser } from '@/lib/auth'
import { ok, errors } from '@/lib/responses'

async function getPedidoConTienda(supabase: any, id: string) {
  const { data, error } = await supabase
    .from('pedidos')
    .select('id, cliente_id, total, tiendas ( propietario_id )')
    .eq('id', id)
    .maybeSingle()

  if (error) throw error
  return data
}

function puedeAcceder(pedido: any, auth: any) {
  const isAdmin = auth.perfil?.rol === 'administrador'
  const isCliente = pedido.cliente_id === auth.user.id
  const isTienda = pedido.tiendas?.propietario_id === auth.user.id
  return isAdmin || isCliente || isTienda
}

export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  const { id } = await params
  const auth = await getAuthUser()
  if (!auth) return errors.unauthorized()

  const supabase = await createClient()

  let pedido
  try {
    pedido = await getPedidoConTienda(supabase, id)
  } catch (error: any) {
    return errors.server(error.message)
  }
  if (!pedido) return errors.notFound('Pedido no encontrado.')
  if (!puedeAcceder(pedido, auth)) return errors.forbidden()

  const { data, error } = await supabase
    .from('pagos')
    .select('*, comprobantes ( id, archivo, observacion, created_at )')
    .eq('pedido_id', id)
    .order('created_at', { ascending: false })

  if (error) return errors.server(error.message)

  return ok(data)
}

// El cliente registra un pago (queda pendiente de confirmación por la tienda)
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  const { id } = await params
  const auth = await getAuthUser()
  if (!auth) return errors.unauthorized()

  const supabase = await createClient()

  let pedido
  try {
    pedido = await getPedidoConTienda(supabase, id)
  } catch (error: any) {
    return errors.server(error.message)
  }
  if (!pedido) return errors.notFound('Pedido no encontrado.')
  if (pedido.cliente_id !== auth.user.id) return errors.forbidden()

  const body = await request.json()
  const { metodo_pago_id, referencia, monto } = body

  if (!metodo_pago_id) return errors.validation('metodo_pago_id es requerido.')
  if (!monto || Number(monto) <= 0) return errors.validation('monto debe ser mayor a 0.')

  const { data, error } = await supabase
    .from('pagos')
    .insert({
      pedido_id: id,
      metodo_pago_id,
      referencia: referencia ?? null,
      monto,
      estado: 'pendiente',
    })
    .select()
    .single()

  if (error) return errors.server(error.message)

  return ok(data, 201)
}